...
function renderFulfillablePage(items) {
    ...
    const html = Object.values(orders).map(order => {
        ...
        return order.items.map((item, index) => {
            ...
            // Item specific cells
            // We add a subtle border between items of the same order, but a heavier border between orders (via the 'group' class logic usually)
            return `
...
export function initFulfillable() {
    ['fulfillableLocFilter', 'fulfillableCustomerFilter', 'fulfillableBuFilter'].forEach(id => {
